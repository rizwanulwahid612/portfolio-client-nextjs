...
import { useDeleteExperianceMutation, useExperiancesQuery } from "@/redux/api/experianceApi";
const { Meta } = Card;
...
const ServicePage = () => {
  ...
  const [deleteExperiance] = useDeleteExperianceMutation();
  ...
  const { data, isLoading } = useExperiancesQuery({ ...query });
  const services = data?.experiance;
      ...
      render: function (data: any) {
        ...
        return (
          <>
            ...
            <Link href={`/user/experiance/edit/${data}`}>
              <Button
                style={{
                  margin: "0px 5px",
                }}
    ...
    <div style={{ margin: '10px' }}>
      <EMBreadCrumb
        items={[
          {
            label: "Experiance",
            link: "/user/experiance",
          },
        ]}
      />
      <ActionBar title="Experiance List">
        <Input
          size="large"
          placeholder="Search"
        ...
        />
        <div>
          <Link href="/user/experiance/create">
            <Button type="primary">Create Experiance</Button>
          </Link>
     ...
     <EMTable
      ...
      >
        <p className="my-5">Do you want to remove this Experiance?</p>
      </EMModal>
    </div>
  );
};
